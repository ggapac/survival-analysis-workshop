...
cum_haz_fit <- survfit(surv_obj ~ treatment, data = dat, type = "fleming")

# Plot the cumulative hazard functions
ggsurvplot(cum_haz_fit, data = dat, fun = "cumhaz", conf.int = TRUE,
           ggtheme = theme_minimal(), 
           title = "Cumulative Hazard Curves by Treatment",
...
```{r}
log_rank_test <- survdiff(Surv(time, status) ~ treatment, data = dat)
print(log_rank_test)
```
